...
public class AccursedArrowProjectileWhileProjectileFlyingTickProcedure {
		...
		if (!world.getEntitiesOfClass(Monster.class, AABB.ofSize(new Vec3(x, y, z), 15, 15, 15), e -> true).isEmpty()) {
			...
			radius = 15;
			{
				...
				for (Entity entityiterator : _entfound) {
					if (flag && (entityiterator == ((Entity) world.getEntitiesOfClass(Monster.class, AABB.ofSize(new Vec3(x, y, z), 15, 15, 15), e -> true).stream().sorted(new Object() {
						...
						ydir = (entityiterator.getY() + entityiterator.getEyeHeight()) - immediatesourceentity.getY();
