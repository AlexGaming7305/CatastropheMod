...
public class SeaConchRightclickedProcedure {
		...
		if (entity.isShiftKeyDown() == false) {
			if ((entity.getCapability(CatastropheModModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CatastropheModModVariables.PlayerVariables())).SoulPower < (entity
				...
				if (world instanceof ServerLevel _level) {
					Entity entityToSpawn = CatastropheModModEntities.HERMIT_CRAB_MINION.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("item.goat_horn.sound.0")), SoundSource.NEUTRAL, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("item.goat_horn.sound.0")), SoundSource.NEUTRAL, 1, 1, false);
					}
				}
			...
			if (((Entity) world.getEntitiesOfClass(HermitCrabMinionEntity.class, AABB.ofSize(new Vec3(x, y, z), 100, 100, 100), e -> true).stream().sorted(new Object() {
				...
				if (!((Entity) world.getEntitiesOfClass(HermitCrabMinionEntity.class, AABB.ofSize(new Vec3(x, y, z), 100, 100, 100), e -> true).stream().sorted(new Object() {
					...
					((Entity) world.getEntitiesOfClass(HermitCrabMinionEntity.class, AABB.ofSize(new Vec3(x, y, z), 100, 100, 100), e -> true).stream().sorted(new Object() {
						Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
							return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
						}
