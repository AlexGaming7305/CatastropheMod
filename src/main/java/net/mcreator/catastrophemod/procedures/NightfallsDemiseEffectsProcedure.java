...
public class NightfallsDemiseEffectsProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, DamageSource damagesource, Entity entity, Entity sourceentity) {
		...
		if (sourceentity.getPersistentData().getBoolean("NightfallsDemiseEffects") == true) {
			if (damagesource.is(ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation("catastrophe_mod:demise")))) {
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						...
						_level.playLocalSound((entity.getX()), (entity.getY()), (entity.getZ()), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("catastrophe_mod:axe_hits")), SoundSource.PLAYERS, 1, 1, false);
					}
				}
			}
		}
