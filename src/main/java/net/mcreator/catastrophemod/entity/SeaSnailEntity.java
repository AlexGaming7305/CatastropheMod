...
public class SeaSnailEntity extends PathfinderMob implements GeoEntity {
	...
	public SeaSnailEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(CatastropheModModEntities.SEA_SNAIL.get(), world);
	}

	public SeaSnailEntity(EntityType<SeaSnailEntity> type, Level world) {
		super(type, world);
		xpReward = 3;
		setNoAi(false);
			...
			public void tick() {
				...
				if (this.operation == MoveControl.Operation.MOVE_TO && !SeaSnailEntity.this.getNavigation().isDone()) {
					...
					if (SeaSnailEntity.this.isInWater()) {
						...
						SeaSnailEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					SeaSnailEntity.this.setSpeed(0);
					SeaSnailEntity.this.setYya(0);
					SeaSnailEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "sea_snail_texture");
	}
	...
	public void baseTick() {
		super.baseTick();
		SeaSnailOnEntityTickUpdateProcedure.execute(this);
	...
	public static void init() {
		SpawnPlacements.register(CatastropheModModEntities.SEA_SNAIL.get(), SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.sea_snail.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.sea_snail.idle"));
		}
