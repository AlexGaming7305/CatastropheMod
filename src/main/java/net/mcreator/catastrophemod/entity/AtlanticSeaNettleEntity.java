...
public class AtlanticSeaNettleEntity extends Monster implements GeoEntity {
	...
	public AtlanticSeaNettleEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(CatastropheModModEntities.ATLANTIC_SEA_NETTLE.get(), world);
	}

	public AtlanticSeaNettleEntity(EntityType<AtlanticSeaNettleEntity> type, Level world) {
		super(type, world);
		xpReward = 5;
		setNoAi(false);
			...
			public void tick() {
				...
				if (this.operation == MoveControl.Operation.MOVE_TO && !AtlanticSeaNettleEntity.this.getNavigation().isDone()) {
					...
					if (AtlanticSeaNettleEntity.this.isInWater()) {
						...
						AtlanticSeaNettleEntity.this.setSpeed(f1 * 0.05F);
					}
				} else {
					AtlanticSeaNettleEntity.this.setSpeed(0);
					AtlanticSeaNettleEntity.this.setYya(0);
					AtlanticSeaNettleEntity.this.setZza(0);
				}
			}
		};
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "atlantic_sea_nettle_texture");
	}
	...
	public void baseTick() {
		super.baseTick();
		AtlanticSeaNettleOnEntityTickUpdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.atlantic_sea_nettle.idle"));
		}
